...
class Log():
    ...
    '''
    def log_write_file_and_console(self):
        logger = logging.getLogger(__name__)
        logger.setLevel(level = logging.INFO)
        # 判断当前日志对象中是否有处理器，如果没有，则添加处理器
        if not logger.handlers:
...
if __name__ == '__main__':
    ...
    # 日志文件大小
    Log(f"日志文件大小:{int(log_file_size)}kb").log_console()
    # 如果文件大于10M则删除旧日志文件
    if log_file_size>10485760:
        os.remove(log_file)
        Log("文件过大，删除旧日志文件").log_console()
    # Log("这是写入内容").log_console()
    # Log("这是写入内容").log_write_file()
    # Log("这是写入内容").log_write_file_and_console()